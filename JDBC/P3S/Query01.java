...
class QueryEvent implements ActionListener {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == cff.jb1) {
			...
			try {
				Connection c = DriverManager.getConnection(Connect.dsnName, "root", "root");
		...
		if (ae.getSource() == cff.jb2) {
			...
			// System.out.println("~~"+sel[0]+" "+sel[1]+" "+sel[2]+" "+sel[3]+" "+sel[4]+"
		...
		if (ae.getSource() == cff.jb3) {
			...
			// "+sel[4]+" "+sel[5]+" "+sel[6]+" "+sel[7]+" "+sel[8]+" "+sel[9]+" "+" ~~ ");
...
class TableResult {
	...
	TableResult(StringBuffer sb) {
		try {
			c = DriverManager.getConnection(Connect.dsnName, "root", "root");
	...
	void showTable() {
		try {
			...
			while (rs.next()) {
				...
				// System.out.print(tdb.rowDataTemp+" @@ "+tdb.rowDataTemp.size()+"
				// "+tdb.rowDataTemp.capacity());
				// System.out.println("~~ "+tdb.rowDataMain+" ~~ "+tdb.rowDataMain.size()+"
...
class Connect {
	...
	public void connectDSN() {
		...
		try {
			...
			Connection c = DriverManager.getConnection(dsnName, "root", "root");
